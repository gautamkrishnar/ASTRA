						...
						while (path != last) {
							if (path === data.path && !data.link) {
								...
								for (i = 0; i < actions.length; i++) {
									...
									if (img.attr('src') !== OC.imagePath('core', 'actions/share')) {
										img.attr('src', image);
	...
	*/},
	...
	share:function(itemType, itemSource, shareType, shareWith, permissions, itemSourceName, expirationDate, callback) {
		...
		if (typeof callback === 'undefined' &&
			...
			var sharedVerify	=	false;
			
				//$.each(OC.Share.statuses, function(item, data) {
					if(!sharedVerify)
					{
						if (typeof OC.Share.statuses[itemSource] === 'undefined') {
							sharedVerify	=	true;
				...
				$.post(OC.filePath('core', 'ajax', 'share.php'),
					{
						...
						if (result && result.status === 'success') {
							if (callback) {	
							 OC.Share.statuses[itemSource]	=	{"link":true};
	...
	showDropDown:function(itemType, itemSource, appendTo, link, possiblePermissions, filename) {
		...
		if (possiblePermissions & OC.PERMISSION_SHARE) {
			...
			/* ALOK html += '<label for="shareWith" class="hidden-visually">'+t('core', 'Share')+'</label>';
			...
			html += '<input id="shareWith" type="hidden" placeholder="'+t('core', 'Share with user or group …')+'" />';
			//html += '<span class="shareWithLoading icon-loading-small hidden"></span>';
			//html += '<ul id="shareWithList">';
			//html += '</ul>';
			...
			if (link && linksAllowed) {
				...
				html += '<input type="checkbox" name="linkCheckbox" class="hidden-visually" id="linkCheckbox"  value="1"  /><label for="linkCheckbox"  class="hidden-visually">'+t('core', 'Share link')+'</label>';
				//html += '<br />';
				...
				html += '<input id="linkText" class="hidden-visually" type="text" readonly="readonly" />';
				html += '</div>';
				...
				if (mailPublicNotificationEnabled === 'yes') {
					...
					html += '<label for="emailText">'+t('core', 'Share link via email')+'</label><br>';
					html += '<input id="email" style="display:none; width:62%;" value="" placeholder="'+t('core', 'Enter email address')+'" type="email" />';
					html += '<input id="emailButton" style="display:none;" type="submit" value="'+t('core', 'Send')+'" />';
					html += '</form>';
					html +='<div id="checkboxoption" style="cursor:pointer;display:none;">+ Options</div>';
				}
			}
			html +='<div id="checkboxDiv" style="display:none;">';
			
				html += '<div class="linkShare">';
				html += '<input type="checkbox" name="showPassword" id="showPassword" value="1" /><label for="showPassword">'+t('core', 'Password protect')+'</label>';
...
$(document).ready(function() {
	...
	$(document).on('click', '#checkboxoption', function() {
		OC.Share.hideCheckBox();
	});
	...
	$(document).on('submit', '#dropdown #emailPrivateLink', function(event) {
		...
		if (email != '') {
			...
			$.post(OC.filePath('core', 'ajax', 'share.php'), { action: 'email', toaddress: email, link: link, itemType: itemType, itemSource: itemSource, file: file, expiration: expirationDate},
				...
				if (result && result.status == 'success') {
					//update icon of shared file
					var img =	$('#fileList').find('tr[data-id="'+itemSource+'"] .fileactions .action[data-action="Share"] img');
					//image = OC.imagePath('core', 'actions/public');
					image = OC.imagePath('core', 'actions/share');
					img.attr('src', image);			
